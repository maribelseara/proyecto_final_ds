...
potencia = st.slider('Potencia', min_value=df['Potencia'].min(), max_value=df['Potencia'].max(), step=5.0, value=100.0)
...
puertas=st.number_input('Puertas:', min_value=df['puertas'].min(), max_value=df['puertas'].max(), step=1, value=5)
...
year=st.number_input('Año de matriculación:', min_value=df['Año'].min(), max_value=date.today().year, value=date.today().year)
...
plazas=st.number_input('Plazas:', min_value=1, max_value=9, value=5)
marchas=st.slider('Cantidad de marchas:', min_value=df['Número de marchas'].min(), max_value=df['Número de marchas'].max(), step=1.0, value=6.0)
